...
import {IconArrowLeft, IconRefresh} from '../common/Icons';
...
class AddEditMqttClient extends Component {
    ...
    initMqttClientObj(params) {
        if(params!=null && params.mcsId!=null && params.mcsId.trim().length>0) {
            ...
            if(this.props.location!=null && this.props.location.query!=null && this.props.location.query.folderId!=null) {
                this.state.folderId = this.props.location.query.folderId;
            }
        }
    }
    ...
    onProtocolValueChange(event) {
        if(PlatformConstants.PLATFORM_TYPE == CommonConstants.PLATFORM_WEB_APP && (event.target.value=='mqtt' || event.target.value=='mqtts')) {
            CommonActions.showMessageToUser({message:'MQTT over TCP and TLS are supported by MQTTBoard apps for Linux, MAC and Windows. Download MQTTBoard app from http://workswithweb.com/mqttbox.html'});
        } else if(PlatformConstants.PLATFORM_TYPE == CommonConstants.PLATFORM_CHROME_APP && event.target.value=='mqtts') {
            CommonActions.showMessageToUser({message:'MQTT over TLS is supported by MQTTBoard apps for Linux, MAC and Windows. Download MQTTBoard app from http://workswithweb.com/mqttbox.html'});
        } else {
            var newState = {};
            newState[event.target.name] = event.target.value;
            this.setState(newState);
    ...
    render() {
        ...
        if(this.props.params.mcsId!=null) {
            ...
            backButton = <a style={{marginLeft: 10}} className="btn btn-default" href={"#/mqttclientdashboard/"+this.props.params.mcsId}><IconArrowLeft size={14}/></a>;
        } else {
            backButton = <a style={{marginLeft: 10}} className="btn btn-default" href="#/mqttclientslist"><IconArrowLeft size={14}/></a>;
        }
                ...
                <nav className="navbar navbar-default navbar-fixed-top">
                    <div>
                        <div className="navbar-header" style={{paddingTop: 8}}>
                            {backButton}
                            <span style={{marginLeft: 10}}><b> MQTT CLIENT SETTINGS</b></span>
                        </div>
                        ...
                        <div className="col-xs-12 col-sm-6 col-md-3">
                            <div className="form-group">
                                ...
                                <div className="input-group">
                                    ...
                                    style={{cursor:'pointer'}} onClick={this.generateMqttClientId} className="input-group-addon"><IconRefresh size={14}/></div>
                                </div>
                            </div>
                        </div>
                        <div className="col-xs-12 col-sm-6 col-md-3">
