...
const DEFAULT_CARD_GRID_X = 0;
const DEFAULT_CARD_GRID_Y = 660;
...
class MqttClientService extends Events.EventEmitter {
        ...
        AppDispatcher.register(function(action) {
            switch(action.actionType) {
                ...
                case MqttClientConstants.ACTION_MQTT_CLIENT_CONNECT:
                    delete this.disconnectingClients[action.data.mcsId];
                ...
                case MqttClientConstants.ACTION_UN_SUBSCRIBE_TO_TOPIC:
                    ...
                    break;
                case MqttClientConstants.ACTION_RENAME_MQTT_CLIENT:
                    this.renameMqttClient(action.data);
                    break;
                case MqttClientConstants.ACTION_MOVE_MQTT_CLIENT:
                    this.moveMqttClient(action.data);
                    break;
                case MqttClientConstants.ACTION_REORDER_MQTT_CLIENT:
                    this.reorderMqttClient(action.data);
                    break;
                case MqttClientConstants.ACTION_UPDATE_MQTT_CLIENT_LAYOUT:
                    this.updateMqttClientLayout(action.data);
                    break;
    ...
    processEvents(eventObj) {	
        switch(eventObj.event) {
            ...
            case MqttClientConstants.EVENT_MQTT_CLIENT_PUBLISHED_MESSAGE:
                ...
                break;
            case MqttClientConstants.EVENT_MQTT_CLIENT_CONNECTION_CLOSED:
                if(eventObj.data != null && eventObj.data.mcsId != null) {
                    delete this.disconnectingClients[eventObj.data.mcsId];
                }
                break;
            default:
        }
    ...
    syncMqttClientStateCache(connStateObj) { 
        if(this.disconnectingClients[connStateObj.mcsId] === true) {
            return;
        }
    ...
    saveMqttClientSettings(data) { 
        ...
        if(dbClientSettingsObj==null) {
            ...
            var defaultPublisher = new PublisherSettings();
            var defaultSubscriber = new SubscriberSettings();
            this.assignNextCardPosition(dbClientSettingsObj, defaultPublisher);
            dbClientSettingsObj.publishSettings.push(defaultPublisher);
            this.assignNextCardPosition(dbClientSettingsObj, defaultSubscriber);
            dbClientSettingsObj.subscribeSettings.push(defaultSubscriber);

            if(data.folderId != null) {
                var clientsInFolder = _.filter(_.values(this.mqttClientSettings), function(client) {
                    return client.folderId === data.folderId;
                });
                var maxOrder = clientsInFolder.length > 0 ? (_.maxBy(clientsInFolder, 'order') || {}).order || 0 : -1;
                dbClientSettingsObj.order = maxOrder + 1;
            }
        }
        ...
        if(data.folderId !== undefined) {
            dbClientSettingsObj.folderId = data.folderId;
        }
        if(data.order !== undefined) {
            dbClientSettingsObj.order = data.order;
        }
        if(data.snapToGridEnabled !== undefined) {
            dbClientSettingsObj.snapToGridEnabled = data.snapToGridEnabled;
        }

        this.mqttClientSettings[data.mcsId] = dbClientSettingsObj;
        MqttClientDbService.saveMqttClientSettings(dbClientSettingsObj);
        if(dbClientSettingsObj.autoConnectOnAppLaunch == true) {
    ...
    disconnectMqttClient(action) { 
        this.disconnectingClients[action.data] = true;
        PlatformDispatcherService.dispatcherAction(action,CommonConstants.SERVICE_TYPE_MQTT_CLIENTS);
    ...
    savePublisherSettings(mcsId,publisher) { 
        ...
        if(obj!=null && publisher!=null) {
            ...
            if(pubIndex!=-1) {
                ...
                isNew = true;
                this.assignNextCardPosition(obj, publisher);
    ...
    saveSubscriberSettings(mcsId,subscriber) { 
        ...
        if(obj!=null && subscriber!=null) {
            ...
            if(subIndex!=-1) {
                ...
                isNew = true;
                this.assignNextCardPosition(obj, subscriber);
    ...
    assignNextCardPosition(mqttClient, cardSettings) {
        var occupiedRows = {};
        var cards = [];

        if(mqttClient.publishSettings != null) {
            cards = cards.concat(mqttClient.publishSettings);
        }
        if(mqttClient.subscribeSettings != null) {
            cards = cards.concat(mqttClient.subscribeSettings);
        }

        for(var i=0;i<cards.length;i++) {
            var rowIndex = Math.max(0, Math.round(Number(cards[i].posY || 0) / DEFAULT_CARD_GRID_Y));
            if(Number(cards[i].posX || 0) === DEFAULT_CARD_GRID_X) {
                occupiedRows[rowIndex] = true;
            }
        }

        var nextRowIndex = 0;
        while(occupiedRows[nextRowIndex] === true) {
            nextRowIndex++;
        }

        cardSettings.posX = DEFAULT_CARD_GRID_X;
        cardSettings.posY = nextRowIndex * DEFAULT_CARD_GRID_Y;
    }
    ...
    renameMqttClient(data) {
        var obj = this.mqttClientSettings[data.mcsId];
        if(obj!=null) {
            obj.mqttClientName = data.name;
    ...
    reorderMqttClient(data) {
        if(data.mcsIds!=null && data.mcsIds.length>0) {
            for(var i=0;i<data.mcsIds.length;i++) {
                var obj = this.mqttClientSettings[data.mcsIds[i]];
                if(obj!=null) {
                    obj.order = i;
                    MqttClientDbService.saveMqttClientSettings(obj);
                }
            }
            this.emitChange(MqttClientConstants.EVENT_MQTT_CLIENT_DATA_CHANGED,data.mcsIds[0]);
        }
    }

    updateMqttClientLayout(data) {
        var obj = this.mqttClientSettings[data.mcsId];
        if(obj!=null) {
            if(data.snapToGridEnabled !== undefined) {
                obj.snapToGridEnabled = data.snapToGridEnabled;
            }
            obj.updatedOn = +(new Date());
            MqttClientDbService.saveMqttClientSettings(obj);
            this.emitChange(MqttClientConstants.EVENT_MQTT_CLIENT_DATA_CHANGED,data.mcsId);
    ...
    toggleFavoriteMqttClient(mcsId) {
        var obj = this.mqttClientSettings[mcsId];
        if(obj != null) {
    ...
    getClientsByFolderId(folderId) {
        var all = _.values(this.mqttClientSettings);
        if(folderId==null) {
            return _.sortBy(all, ['order']);
        }
        return _.sortBy(_.filter(all, function(c) { return c.folderId === folderId; }), ['order']);
    }
}
